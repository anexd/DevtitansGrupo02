...
MODULE_AUTHOR("DevtitansGrupo02");
MODULE_DESCRIPTION("Driver de acesso ao ScanWifi (ESP32 com Chip Serial CP2102");
MODULE_LICENSE("GPL");
...
// Executado quando o arquivo /sys/kernel/scanwifi/{wifi , threshold} é lido (e.g., cat /sys/kernel/scanwifi/led)
static ssize_t attr_show(struct kobject *sys_obj, struct kobj_attribute *attr, char *buff);
// Executado quando o arquivo /sys/kernel/scanwifi/{wifi , threshold} é escrito (e.g., echo "100" | sudo tee -a /sys/kernel/scanwifi/led)
...
static struct usb_device *scanwifi_device;        // Referência para o dispositivo USB
static uint usb_in, usb_out;                       // Endereços das portas de entrada e saida da USB
static char *usb_in_buffer, *usb_out_buffer;       // Buffers de entrada e saída da USB
static int usb_max_size;                           // Tamanho máximo de uma mensagem USB

// Variáveis para criar os arquivos no /sys/kernel/scanwifi/{wifi , threshold}

static struct kobj_attribute  wifi_attribute = __ATTR(wifi, S_IRUGO | S_IWUSR, attr_show, attr_store);
static struct attribute      *attrs[]       = { &wifi_attribute.attr, NULL };
...
static struct usb_driver scanwifi_driver = {
    .name        = "scanwifi",     // Nome do driver
    .probe       = usb_probe,       // Executado quando o dispositivo é conectado na USB
    .disconnect  = usb_disconnect,  // Executado quando o dispositivo é desconectado na USB
    .id_table    = id_table,        // Tabela com o VendorID e ProductID do dispositivo
};
module_usb_driver(scanwifi_driver);

// Executado quando o dispositivo é conectado na USB
static int usb_probe(struct usb_interface *interface, const struct usb_device_id *id) {
    struct usb_endpoint_descriptor *usb_endpoint_in, *usb_endpoint_out;

    printk(KERN_INFO "scanwifi: Dispositivo conectado ...\n");

    // Cria arquivos do /sys/kernel/scanwifi/*
    sys_obj = kobject_create_and_add("scanwifi", kernel_kobj);
    ignore = sysfs_create_group(sys_obj, &attr_group); // AQUI

    // Detecta portas e aloca buffers de entrada e saída de dados na USB
    scanwifi_device = interface_to_usbdev(interface);
...
static void usb_disconnect(struct usb_interface *interface) {
    printk(KERN_INFO "scanwifi: Dispositivo desconectado.\n");
    if (sys_obj) kobject_put(sys_obj);      // Remove os arquivos em /sys/kernel/scanwifi
...
static char* usb_send_cmd(char *cmd, int param) {
    ...
    printk(KERN_INFO "scanwifi: Enviando comando: %s\n", cmd);
    ...
    ret = usb_bulk_msg(scanwifi_device, usb_sndbulkpipe(scanwifi_device, usb_out), usb_out_buffer, strlen(usb_out_buffer), &actual_size, 1000);
    if (ret) {
        printk(KERN_ERR "scanwifi: Erro de codigo %d ao enviar comando!\n", ret);
        return "error";
    }
    ...
    while (retries > 0) {
        // Lê dados da USB
        ret = usb_bulk_msg(scanwifi_device, usb_rcvbulkpipe(scanwifi_device, usb_in), usb_in_buffer, min(usb_max_size, MAX_RECV_LINE), &actual_size, 1000);
        if (ret) {
            printk(KERN_ERR "scanwifi: Erro ao ler dados da USB (tentativa %d). Codigo: %d\n", ret, retries--);
            continue;
        }
        ...
        for (i=0; i<actual_size; i++) {
            ...
            if (usb_in_buffer[i] == '\n') {  // Temos uma linha completa
                ...
                printk(KERN_INFO "scanwifi: Recebido uma linha: '%s'\n", recv_line);
                ...
                if (!strncmp(recv_line, resp_expected, strlen(resp_expected))) {
                    printk(KERN_INFO "scanwifi: Linha eh resposta para %s! Processando ...\n", cmd);
...
static ssize_t attr_show(struct kobject *sys_obj, struct kobj_attribute *attr, char *buff) {
    ...
    printk(KERN_INFO "scanWifi: Lendo %s ...\n", attr_name);

    if (!strcmp(attr_name, "wifi"))
    {
        value = usb_send_cmd("GET_WIFI", -1);
    }else {
        printk(KERN_ALERT "scanwifi: comando invalido.\n");
        return -EACCES;
    }
